...
class ProveedorQueryWindow(QWidget):
    def __init__(self, go_back_callback):
        ...
        main_layout.addWidget(QLabel("Consultar Proveedores"))

        search_layout = QHBoxLayout()
        search_layout.addWidget(QLabel("Buscar por Nombre:"))
        self.search_box = QLineEdit()
        self.search_box.setPlaceholderText("Dejar vacío para mostrar todos...")
    ...
    def load_data(self):
        ...
        if not search_term:
            results = DB_GLOBAL.get_vista_proveedores()
        else:
            results = DB_GLOBAL.search_proveedor(search_term)
    ...
    def confirm_changes(self):
        ...
        for pk, change in self.pending_changes.items():
            summary += f"Proveedor ID {pk}, Columna '{change['column_header']}': Nuevo Valor '{change['value']}'\n"
    ...
    def save_changes_to_db(self):
        try:
            for pk, change in self.pending_changes.items():
                DB_GLOBAL.update_supplier(pk, change['column_header'], change['value'])
