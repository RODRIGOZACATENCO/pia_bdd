...
class TurnoQueryWindow(QWidget):
    def __init__(self, go_back_callback):
        ...
        main_layout.addWidget(QLabel("Consultar Turnos"))
    ...
    def load_data(self):
        ...
        results = DB_GLOBAL.get_vista_turnos()
        headers = ["ID", "Nombre", "Hora Inicio", "Hora Fin", "N. Empleados"]
    ...
    def populate_table(self, headers, data):
        ...
        for row_num, row_data in enumerate(data):
            for col_num, item in enumerate(row_data):
                ...
                if col_num == 0 or col_num == 4:  # ID or N. Empleados (integer)
    ...
    def confirm_changes(self):
        ...
        for pk, change in self.pending_changes.items():
            summary += f"Turno ID {pk}, Columna '{change['column_header']}': Nuevo Valor '{change['value']}'\n"
    ...
    def save_changes_to_db(self):
        try:
            for pk, change in self.pending_changes.items():
                DB_GLOBAL.update_shift(pk, change['column_header'], change['value'])
