...
class ProductTypeInputWindow(QWidget):
    def __init__(self, go_back_callback):
        ...
        main_layout.addWidget(QLabel("Registrar Nuevo Tipo de Producto"))

        self.form_layout = QFormLayout()
        self.name_input = QLineEdit()
        self.name_input.setPlaceholderText("Ej: Cerveza, Refresco, Tequila")

        self.form_layout.addRow("Nombre:", self.name_input)
        main_layout.addLayout(self.form_layout)

        self.register_button = QPushButton("Registrar Tipo")
        self.register_button.clicked.connect(self.register_product_type)
    ...
    def register_product_type(self):
        ...
        try:
            new_id = DB_GLOBAL.add_new_product_type(name)
            if new_id:
                QMessageBox.information(self, "Éxito", f"Tipo de producto '{name}' registrado con ID: {new_id}.")
                self.name_input.clear()
            else:
                QMessageBox.critical(self, "Error", "No se pudo registrar el tipo de producto.")
        except Exception as e:
